...
export class PLSQLExecutor {
    ...
    constructor(connection: Connection, debugMode: boolean = false) {
        ...
        try {
            ...
            for (const param of detectedOutParams) {
                if (!(param in finalBinds)) {
                    finalBinds[param] = { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 4000 };
                }
            }
    ...
    async executeProcedure(
        ...
        const paramList = Object.keys(parameters).map(param => `:${param}`).join(', ');
    ...
    async executeFunction(
        ...
        const configuredBinds = {
            result: { dir: oracledb.BIND_OUT, type: this.getOracleType(returnType), maxSize: 4000 },
            ...parameters
        };
